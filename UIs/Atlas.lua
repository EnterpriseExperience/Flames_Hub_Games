...
do
    function Library.new(info)
        ...
        local function makeLibrary()
            ...
            Converted["_Discord"].Value = "https://discord.gg/MTYKxQfpNJ"
...
do
    function Section.new(_self,info) -- used internally and has no argument checks, use Page::CreateSection instead
        ...
        local function make_section()
            ...
            Converted["_Contents"].Size = UDim2.new(1, -27, 0, 0)
            Converted["_Contents"].Name = "Contents"
    ...
    function Section:CreateToggle(...) return Element.CreateToggle(self, ...) end
    function Section:CreateSlider(...) return Element.CreateSlider(self, ...) end
    function Section:CreateSliderToggle(...) return Element.CreateSliderToggle(self, ...) end
    ...
    function Section:CreateBody(...) return Section:CreateParagraph(...) end
    function Section:CreateButton(...) return Element.CreateButton(self, ...) end
    function Section:CreateTextBox(...) return Element.CreateTextBox(self, ...) end
    function Section:CreateInteractable(...) return Element.CreateInteractable(self, ...) end
    function Section:CreateKeybind(...) return Element.CreateKeybind(self, ...) end
    function Section:CreateKeyBind(...) return Section:CreateKeybind(...) end
    function Section:CreateDropdown(...) return Element.CreateDropdown(self, ...) end
    function Section:CreateColorPicker(...) return Element.CreateColorPicker(self, ...) end
end
...
do
    function Element.CreateToggle(section,info)
        ...
        if _self._usedFlags[info.Flag] then return else _self._usedFlags[info.Flag] = info.SavingDisabled and "disabled" or true end
        if info.Default == nil then info.Default = false end
        if _self.Flags[info.Flag]==nil then _self.Flags[info.Flag] = info.Default end
        if info.SavingDisabled then _self.Flags[info.Flag] = info.Default end
        info.Callback = info.Callback or utility.BlankFunction
        if info.CallbackOnCreation then coroutine.wrap(info.Callback)(_self.Flags[info.Flag]) end
        ...
        function toggle_object:Set(new_value, fire_callback)
            ...
            if fire_callback then coroutine.wrap(info.Callback)(new_value) end
        end
        function toggle_object:Get() return _self.Flags[info.Flag] end
    ...
    function Element.CreateSlider(section,info)
        ...
        if _self._usedFlags[info.Flag] then return else _self._usedFlags[info.Flag] = true end
        if info.Default==nil then info.Default = info.Min end
        if info.AllowOutOfRange==nil then info.AllowOutOfRange = false end
        if info.Digits==nil then info.Digits = 0 end
        info.Callback = info.Callback or utility.BlankFunction
        if _self.Flags[info.Flag]==nil then _self.Flags[info.Flag] = info.Default end
        if info.SavingDisabled then _self.Flags[info.Flag] = info.Default end
        if info.CallbackOnCreation then coroutine.wrap(info.Callback)(_self.Flags[info.Flag]) end
...
if Library and Library.new and typeof(Library.new) == "function" then getgenv().New_Atlas = Library.new end
return Library